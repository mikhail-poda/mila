...
class SerializerWin implements ISerializer {
  static Future<ISerializer> load(String dirName) async {
    var file = p.join(dirName, "0. vocabulary.txt");
    var lines = await PlatformWin.loadAsync(file).toList();

    return SerializerWin(lines, file);
  }

  late File _file;
  late Map<String, Kvp> _map;

  SerializerWin(List<List<String>> lines, String fpath) {
